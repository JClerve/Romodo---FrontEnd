...
const Vehicles: React.FC = () => {
  ...
  const handleDeleteConfirmation = () => {
    ...
    const v = vehicles.filter((v) => v.id === selected[0])[0];
    axios
      .delete(`http://localhost:5000/api/v1/vehicle/${v.id}`)
      .then((r) => {
        if (r.status === 204) {
  ...
  const onChangeAvailability = (availability: boolean, vehicle: Vehicle) => {
    axios
      .put(`http://localhost:5000/api/v1/vehicle/${vehicle.id}`, {
        availability,
      })
      .then(() => {
        getAllVehicles();
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (filteredVehicles.length === 0) {
      setErrorMessage("Cannot find the vehicle with the given ID");
    } else {
      setErrorMessage("");
    }
  };
