...
const Drivers: React.FC = () => {
  ...
  const getAllDrivers = () => {
    ...
    fetch("http://localhost:5000/api/v1/driver")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDeleteConfirmation = () => {
    ...
    axios
      .delete(`http://localhost:5000/api/v1/driver/${d._id}`)
      .then((r) => {
        if (r.status === 204) {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (filteredDrivers.length === 0) {
      setErrorMessage("Cannot find the driver with the given number");
    } else {
      setErrorMessage("");
    }
  };
  ...
  const handleStatusChange = (id: string, isActive: boolean) => {
    axios
      .put(`http://localhost:5000/api/v1/driver/${id}/status`, { isActive: !isActive })
      .then((response) => {
        console.log(
  ...
  const onChangeAvailability = (availability: boolean, driver: Driver) => {
    axios
      .put(`http://localhost:5000/api/v1/driver/${driver._id}`, {
        availability,
      })
      .then(() => {
        getAllDrivers();
