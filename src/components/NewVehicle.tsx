  ...
  function NewVehicle(props: NewDriverProps) {
	...
	const handleAddVehicle = () => {
	  ...
	  if (checkValidation()) {
		...
		axios
		  .post("http://localhost:5000/api/v1/vehicle", vehicle)
		  .then((response) => {
	...
	const handleUpdateVehicle = () => {
	  ...
	  if (checkValidation()) {
		...
		axios
		  .put(`http://localhost:5000/api/v1/vehicle/${id}`, vehicle)
		  .then((response) => {
	...
	return (
	  <div>
		...
		>
		  <DialogTitle id="alert-dialog-title">{"Vehicle Details"}</DialogTitle>
		  <DialogContent>
			<br />
			  ...
			  <Grid container spacing={2}>
				...
				<Grid item xs={12} sm={6}>
				  <TextField
					...
					value={noOfSeats}
					onChange={(event) =>
					  setNoOfSeats(parseInt(event.target.value))
					}
					required
				  />
				</Grid>
		  ...
		  <DialogActions>
			...
			<Button
			  onClick={props.isUpdate ? handleUpdateVehicle : handleAddVehicle}
			>
